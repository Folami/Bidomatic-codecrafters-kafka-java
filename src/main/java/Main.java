    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (requestApiKey == 18) { // ApiVersions request
                if (errorCode != 0) {
                    // Error response: body length = 2 (error_code) +1 (compact array) +4 (throttle_time_ms) +1 (TAG_BUFFER) = 8 bytes.
                    // Overall, correlation id (4 bytes) is prepended, so message_length = 4 + 8 = 12.
